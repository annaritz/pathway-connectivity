...
def plot_hist(compound_graph,hypergraph,outprefix):
	cgraph_vals = read_file(compound_graph)
	hgraph_vals = read_file(hypergraph)

	fig, (ax1,ax2) = plt.subplots(ncols=2, nrows=1, figsize=(8,5))

	add_ax_hist(fig,ax1,cgraph_vals,'Hypergraph-Transformed Graph Connectivity',nbins=500,broken=True)
	add_ax_hist(fig,ax2,hgraph_vals,'Hypergraph Connectivity',nbins=30,broken=False)
...
if __name__ == '__main__':
	if len(sys.argv) != 4:
		print('USAGE: python3 connectivity-survey.py <GRAPH WITH COMPLEXES> <HYPERGRAPH_NODES>  <OUTPREFIX>')

	plot_hist(sys.argv[1],sys.argv[2],sys.argv[3])